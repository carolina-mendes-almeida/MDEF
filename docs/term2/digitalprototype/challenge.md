...
Who knew challenges were actually challenging? Well, microchallenges are a (big) understatement of what this week looked like. The MOC group decided to stick together and make a portable version of the last machine so we could take it on the El Hierro research trip. We used so many skills… 3D printing, electronic hardware set-ups that included a button + LED + speaker + SD card mp3 player + GPS, laser cutting EVA, GPS data retrieval systems, and turned data into information, a more commonly understandable format by translating coordinates into pins on a map! I am so proud of myself and the whole group for pulling this off.

The MOC has still the same concept but now it is portable, meaning you can get your “stickers” (or versions of this guerrilla system) anywhere in the world. So, we went from the “city of Barcelona is the Museum of Colonization” to “the world is the Museum of Colonization”. It can seem like a careless generalization if one’s fonts of knowledge were only History classes in our sterilized and stiff school curriculums. But, once we get to use some critical thinking and other references, the message is clear. The European expeditions in search of the “New World” initiated the homogenization of the Globe (cultural, ecological, and racial) and started a globalized network known as the Atlantic Slave Trade. So, yes the MOC is global because colonization is a systemic global problem.

So it should be portable… we thought of low-tech ways tickets might be given out - something like stamps. And, thought of how others could see the dimension of a hypothetical (for now) MOC community - GPS and map visualization. Also wanted to add a bit of fun! So, we incorporated a speaker that was similar to the purpose of the [Staples slogan and button](https://www.youtube.com/watch?v=NojiCsZQSr8) “that is easy”, but saying a different sentence: “that is so colonial”. The last one calls attention during the stamping of a colonial mark on a city, but it can also be used to vocalize the depreciation of something being said, for example in a lecture.

Please check out our [GitHub repository](https://github.com/stella-dikmans/MOC-II/blob/main/README.md) to know step by step and the coding behind this machine.

Even though we did not manage to fit all the electronics inside the body of the MOC button, later, while preparing Design Dialogues Exhibition II we managed to do so. And, this is what it looks like with all its components. 

Also, even though we did so much research, trial and error with this MOC Button we still cannot get it to work consistently. It is very frustrating when the code is right, the hardware is well assembled and put together but for some reason it still does not work everytime we need it to. So we have decided not to bring it to El Hierro, Yet, this is not the end of the MOC. Third trimester we’ll be back!